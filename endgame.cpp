...
DB_REC_PTR_T    dbCreate(long nbk, long nwk, long nbp, long nwp, long rankbp, long rankwp);
unsigned long   dbLocbvToSubIdx(DBENTRYPTR * dbentry, stdBoard b);
DB_REC_PTR_T    dbValInit(long nbk, long nwk, long nbp, long nwp, long rankbp, long rankwp);
...
unsigned long RotateBoard(unsigned long vecrot);
int DBrevindex(int *pos, int k);
int DBindex(int *pos, int k);
void SaveIndex(int off);
void InitIndex(int off, int k, int n);
long Nsq(int rank, WHERE_T *Ppos, int nP);
void NextIndex(int off);
void LoadIndex(int off);
...
void DBInit()
{
        ...
        int minn, dbidx, dbsize;
        long int number, index, deftype, addr, byte, limit, blocks;
        char dt, dtt, next;
        int * dbindex, buffers;
        int dbffp;
        FILE * dbifp;
        ...
        DBFile = open( "chinook/DB6", O_BINARY|O_RDONLY);
        if( DBFile == 0 )
        {
        ...
        /*                      Nice compression ratio!!                */
                ...
                MaxBlock = blocks++;
                printf("... allocating %d/%ld entry index\n", dbsize, blocks);
                DBIndex = dbindex  = ( int *)
                                malloc(blocks * sizeof(int));
...
int DBLookup(stdBoard b)
{
        int result, diff, back, fwd, use;
        unsigned int index, cindex, c;
        DBENTRYPTR dbentry;
        int start, end, middle, byte, def;
        ...
        /* Find out its index */
        index = dbLocbvToSubIdx( &dbentry, b );
        ...
        if ((int)index < 0) {
                ...
                if ((int)index == -(TIE + 1))
                        return((int) TIE);
                if ((int)index == DB_UNKNOWN)
                        return((int) DB_UNKNOWN);
                return(index);
        }
        ...
        if( start == (int)dbentry->startaddr )
        {
                i = byte;
                cindex = 0;
        }
        ...
        if (i < 0 || i >= DISK_BLOCK)
        {
                ...
                printf("Start %lu end %lu byte %d = middle %d\n",
                        (dbentry->startaddr),
                        (dbentry->endaddr),
...
DB_REC_PTR_T dbValInit(long nbk, long nwk, long nbp, long nwp, long rankbp, long rankwp)
{
        return(dbCreate(nbk, nwk, nbp, nwp, rankbp, rankwp));
}
...
unsigned long dbLocbvToSubIdx( DBENTRYPTR * dbentry, stdBoard b )
{
  ...
  for (int i = 0;i < 8;++i){
    for (int j = 0;j < 4;++j) {
      DbBoard.pieces[1][(i*4)+j] = b.pieces[0][(28-i*4)+j];
      DbBoard.pieces[0][(i*4)+j] = b.pieces[1][(28-i*4)+j];
      DbBoard.pieces[2][(i*4)+j] = b.pieces[2][(28-i*4)+j];
    }
  }
...
long Nsq(int rank, WHERE_T *Ppos, int nP)
{
        unsigned long pawnhit  = ((7 - rank) << 2);
